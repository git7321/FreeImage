...
const char zip_copyright[] =
   " zip 1.01 Copyright (C) Gilles Vollant ";
...
*/
local int ziplocal_putValue (const zlib_filefunc_def* pzlib_filefunc_def, voidpf filestream,
							 uLong x, int nbByte)
{
    unsigned char buf[4];
    int n;
...
#    ifndef NO_ADDFILEINEXISTINGZIP
    ...
    if (append == APPEND_STATUS_ADDINZIP)
    {
        ...
        if (size_comment>0)
        {
            ziinit.globalcomment = (char*)ALLOC((uInt)(size_comment+1));
...
local int zipFlushWriteBuffer(zip_internal* zi)
{
    ...
    if (zi->ci.encrypt != 0)
    {
        ...
        for (i=0;i<zi->ci.pos_in_buffered_data;i++)
            zi->ci.buffered_data[i] = (Byte)zencode(zi->ci.keys, zi->ci.pcrc_32_tab,
...
ZEXTERN int ZEXPORT zipWriteInFileInZip (zipFile file, const void* buf, unsigned len)
{
    zip_internal* zi;
    int err=ZIP_OK;
