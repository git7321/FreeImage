...
#  define zstrerror(NUM) gz_strwinerror((DWORD)NUM)
#  define zerrno() ((int)GetLastError())
#else
#  include <errno.h>
#  define zseterrno(ERR) do { errno = (ERR); } while (0)
#  define zstrerror(NUM) strerror(NUM)
#  define zerrno() errno
#endif
...
/* Map the Windows error number in ERROR to a locale-dependent error
   ...
   The gz_strwinerror function does not change the current setting
   of GetLastError.  */

local char * __cdecl gz_strwinerror (error)
     DWORD error;
{
...
typedef struct gz_stream {
    ...
    char      mode;        /* 'w' or 'r' */
    z_off_t   start;       /* start of compressed data in file */
...
local gzFile  gz_open      OF((const char *, const char *, int));
    ...
    int  fd;
{
    ...
    zseterrno(0);
    s->file = fd == -1 ? fopen(path, fmode) : (FILE*)fdopen(fd, fmode);
    ...
    gz_stream *s;
{
    ...
    if (s->stream.next_in[0] != (Byte)gz_magic[0] ||
        s->stream.next_in[1] != (Byte)gz_magic[1]) {
        s->transparent = 1;
        return;
    }
    s->stream.avail_in -= 2;
    ...
    while (s->stream.avail_out != 0) {
        ...
        if (s->transparent) {
            ...
            if (s->stream.avail_out > 0 && !feof(s->file)) {
                s->stream.avail_out -=
                    fread(next_out, 1, s->stream.avail_out, s->file);
            }
    ...
    int *errnum;
{
    ...
    *errnum = s->z_err;
    ...
    TRYFREE(s->msg);
    s->msg = ALLOC(strlen(s->path) + strlen(m) + 3);
