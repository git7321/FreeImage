...
const char deflate_copyright[] =
   " deflate 1.2.5 Copyright (C) Jean-loup Gailly ";
...
local uInt read_buf      OF((z_streamp strm, Bytef *buf, unsigned size));
#ifdef ASMV
      void ZEXPORTVA match_init OF((void));
      uInt ZEXPORTVA longest_match OF((deflate_state *s, IPos cur_match));
#else
    ...
    int flush;
{
    ...
    if (s->status == EXTRA_STATE) {
        if (s->gzhead->extra != Z_NULL) {
            ...
            while (s->gzindex < (s->gzhead->extra_len & 0xffff)) {
                if (s->pending == (uInt)s->pending_buf_size) {
    ...
    if (s->status == HCRC_STATE) {
        if (s->gzhead->hcrc) {
            if (s->pending + 2 > (uInt)s->pending_buf_size)
                flush_pending(strm);
            if (s->pending + 2 <= (uInt)s->pending_buf_size) {
...
local uInt read_buf(strm, buf, size)
    z_streamp strm;
    Bytef *buf;
    unsigned size;
{
    ...
    int flush;
{
    ...
    for (;;) {
        ...
        if (s->match_length >= MIN_MATCH) {
            ...
            _tr_tally_dist(s, (ush)1, (uch)(s->match_length - MIN_MATCH), bflush);
