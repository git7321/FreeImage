...
static int __cdecl PaletteCompareColorsForQsort(const void* p1, const void* p2) {
...
static WEBP_INLINE uint32_t HashPix(uint32_t pix) {
  return (uint32_t)(((((uint64_t)pix + (pix >> 19)) * 0x39c5fba7UI64) & 0xffffffffu) >> 24);
}
...
static int GetHuffBitLengthsAndCodes(
  ...
  huff_tree = (HuffmanTree*)WebPSafeMalloc(3UI64 * max_num_symbols,
                                           sizeof(*huff_tree));
