...
static void FTransformPass1_SSE2(const __m128i* const in01,
  ...
  const __m128i tmp0   = _mm_madd_epi16(a01, k88p);
  const __m128i tmp2   = _mm_madd_epi16(a01, k88m);
  ...
  const __m128i s_lo   = _mm_unpacklo_epi16(s03, s12);
  const __m128i s_hi   = _mm_unpackhi_epi16(s03, s12);
  const __m128i v23    = _mm_unpackhi_epi32(s_lo, s_hi);
  *out01 = _mm_unpacklo_epi32(s_lo, s_hi);
  *out32 = _mm_shuffle_epi32(v23, _MM_SHUFFLE(1, 0, 3, 2));
}
...
static void FTransformWHTRow_SSE2(const int16_t* const in, __m128i* const out) {
  ...
  const __m128i A01 = _mm_unpacklo_epi16(src0, src1);
  const __m128i A23 = _mm_unpacklo_epi16(src2, src3);
  const __m128i B0 = _mm_adds_epi16(A01, A23);
  const __m128i B1 = _mm_subs_epi16(A01, A23);
  const __m128i C0 = _mm_unpacklo_epi32(B0, B1);
  const __m128i C1 = _mm_unpacklo_epi32(B1, B0);
  const __m128i D = _mm_unpacklo_epi64(C0, C1);
...
static void CollectHistogram_SSE2(const uint8_t* ref, const uint8_t* pred,
  ...
  for (j = start_block; j < end_block; ++j) {
...

    {
      ...
      const __m128i abs0 = _mm_max_epi16(out0, d0);
