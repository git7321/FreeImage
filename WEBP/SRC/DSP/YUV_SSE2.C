...
static uint64_t SharpYUVUpdateY_SSE2(const uint16_t* ref, const uint16_t* src,
  ...
  for (i = 0; i + 8 <= len; i += 8) {
    ...
    const __m128i D = _mm_sub_epi16(A, B);
    const __m128i E = _mm_cmpgt_epi16(zero, D);
    const __m128i F = _mm_add_epi16(C, D);
    const __m128i G = _mm_or_si128(E, one);
    const __m128i H = _mm_max_epi16(_mm_min_epi16(F, max), zero);
    const __m128i I = _mm_madd_epi16(D, G);
...
static void SharpYUVFilterRow_SSE2(const int16_t* A, const int16_t* B, int len,
  ...
  for (i = 0; i + 8 <= len; i += 8) {
    ...
    const __m128i a0a1b0b1 = _mm_add_epi16(a0b1, a1b0);
    const __m128i a0a1b0b1_8 = _mm_add_epi16(a0a1b0b1, kCst8);
    const __m128i a0b1_2 = _mm_add_epi16(a0b1, a0b1);
    const __m128i a1b0_2 = _mm_add_epi16(a1b0, a1b0);
