    ...
    int nparams, png_const_charp units, png_charpp params)
{
   ...
   length = (png_size_t)png_check_keyword(purpose, new_purpose);
    ...
    png_const_bytep profile, png_uint_32 proflen)
{
   ...
   length = (png_size_t)png_check_keyword(name, new_name);
    ...
    png_const_textp text_ptr, int num_text)
{
  ...
  png_debug1(1, "in %lx storage function", png_ptr == NULL ? 0xabadca11 :
   ...
   for (i = 0; i < num_text; i++)
   {
      ...
      key_len = (png_size_t)png_check_keyword(text_ptr[i].key, new_key);
...
#  ifdef PNG_iTXt_SUPPORTED
      {
         if (text_ptr[i].lang != NULL)
            lang_len = (png_size_t)png_check_keyword(text_ptr[i].lang, new_lang);
    ...
    png_infop info_ptr, png_const_sPLT_tp entries, int nentries)
{
   ...
   for (i = j = 0; i < nentries; i++)
   {
      ...
      length = (png_size_t)png_check_keyword(from->name, new_name);
