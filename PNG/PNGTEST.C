...
static long maximum_allocation = 0;
...
PNGCBAPI png_debug_malloc(png_structp png_ptr, png_alloc_size_t size)
{
      ...
      return (NULL);
   {
      ...
      pinformation = pinfo;
      memset(pinfo->pointer, 0xdd, (size_t)pinfo->size);
      ...
      for (;;)
      {
         ...
         if (pinfo->pointer == ptr)
         {
            *ppinfo = pinfo->next;
            ...
            memset(ptr, 0x55, (size_t)pinfo->size);
            if (pinfo != NULL)
               free(pinfo);
            pinfo = NULL;
            break;
...
#  endif
   ...
   write_chunks(write_ptr, before_PLTE);
   png_write_info(write_ptr, write_info_ptr);
   write_chunks(write_ptr, before_IDAT);
#endif
