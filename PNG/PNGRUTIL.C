...
png_crc_error(png_structp png_ptr)
{
   ...
   if (PNG_CHUNK_ANCILLIARY(png_ptr->chunk_name)!=0)
   {
    ...
    png_size_t prefix_size, png_size_t *newlength)
{
   ...
   else if (comp_type == PNG_COMPRESSION_TYPE_BASE)
   {
      ...
      else if (expanded_size > 0)
      {
         ...
         memset(text, 0, prefix_size + expanded_size + 1);
...
png_handle_unknown(png_structp png_ptr, png_infop info_ptr, png_uint_32 length)
{
       ...
       )
   {
      ...
      else
      {
         ...
         png_crc_read(png_ptr, png_ptr->unknown_chunk.data, (png_size_t)length);
      }
...
#     endif
   ...
   if (png_ptr->interlaced && (png_ptr->transformations & PNG_INTERLACE) &&
      ...
      (display == 1 && (pass & 1) != 0)))
   {
      if (row_width <= (png_uint_32)(PNG_PASS_START_COL(pass)))
         return;

      if (pixel_depth < 8)
      {
#ifdef PNG_USE_COMPILE_TIME_MASKS
...
png_read_filter_row_paeth_multibyte_pixel(png_row_infop row_info, png_bytep row,
   png_const_bytep prev_row)
{
   png_size_t bpp = (row_info->pixel_depth + 7) >> 3;
...
defined(PNG_USER_TRANSFORM_PTR_SUPPORTED)
   ...
   row_bytes = (png_size_t)((png_ptr->width + 7) & ~((png_uint_32)7));
